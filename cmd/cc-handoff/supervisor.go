...
说明:
  overview / queue / read / send / usage 依赖桌面 App 注入的 CC_BUS_DIR。
...
func runSupervisor(ctx context.Context, args []string) error {
	...
	default:
		return fmt.Errorf("unknown supervisor subcommand %q (want init|context|overview|queue|read|send|usage|decide)", sub)
	}
}
