...
func runSent(ctx context.Context, args []string) error {
	...
	items, err := client.ListSent(ctx, *limit)
	if err != nil {
		return relayCompatError(err, "sent")
	}
	if *asJSON {
		return json.NewEncoder(os.Stdout).Encode(items)
	}
	if len(items) == 0 {
		fmt.Println("you have not sent any handoffs yet.")
		return nil
	}
	fmt.Printf("%-32s  %-22s  %-11s  %-7s  %-19s  %s\n", "ID", "TO", "STATE", "URG", "WHEN", "HEADLINE")
	for _, it := range items {
		fmt.Printf("%-32s  %-22s  %-11s  %-7s  %-19s  %s\n",
			it.ID,
			truncRight(it.Recipient, 22),
			it.State,
			it.Urgency,
