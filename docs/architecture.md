...
- **目标正忙(mid-turn)** → 写一份 `$CC_BUS_DIR/inbox/<session-id>/<seq>.json`
  (`internal/localbus`,原子 tmp+rename、FIFO),由目标会话的 **PostToolUse hook**
  在下一个工具边界把消息当 `additionalContext` 注入当前 turn(**不打断**);若该 turn
  不再调工具,则其 **Stop hook** 在结束时兜底(`{"decision":"block"}` 拉一个新 turn)。
  两者都跑 `cc-handoff bus-hook`,清 marker 防循环;`Stop` 且 `stop_hook_active` 时
  直接 bail(不 drain,留给下一次)以免丢消息。
...
契约一致(`additionalContext`/`decision:block`),同一个 `cc-handoff bus-hook` 通吃;App 启动时
