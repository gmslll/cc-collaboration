...
- **Mac** 上每个仓库装一份 `cc-handoff` CLI + 一份 `cc-handoff-mcp`（Claude Code 通过 stdio 拉起的 MCP server）
- **接收侧 Mac** 额外起一个 `cc-handoff watch` 守护进程（launchd），SSE 长连接拉服务端事件、落盘到 `.claude/handoff-inbox/`、必要时弹通知或开新终端
...
#   Register MCP server with Claude Code (user scope)? [Y/n]: <回车>
#   Copy /handoff /handoff-module /pickup slash commands? [Y/n]: <回车>
```

`--with-mcp` 等价于 2.3 的脚本,`--with-commands` 等价于 2.4 的拷贝(还会带版本戳,
将来 init 二跑会提示哪些命令旧了)。两个 flag 都可以省 — 省了就退化成只写 toml,
2.3 / 2.4 自己做。还可以加 `--no-mcp` / `--no-commands` 显式跳过 prompt;
脚本化场景配 `--non-interactive` 用。
...
把 `cc-handoff-mcp` 注册到 Claude Code，让 Claude 看得到 `submit_handoff` / `list_inbox` / `pickup_handoff` / `comment_handoff` 四个工具：
...
实际就是 `claude mcp add --transport stdio cc-handoff -- /usr/local/bin/cc-handoff-mcp`，幂等（已存在时先 remove 再 add）。`cc-handoff init --with-mcp` 走的是同一条逻辑(从 Go 调,不依赖 shell 脚本所在路径)。
...
**重启 Claude Code session**（让它重新发现 MCP 工具）。然后随便开一个 Claude Code 会话：

```
> 我现在有哪些 MCP 工具？
```

Claude 应当列出 `submit_handoff` / `list_inbox` / `pickup_handoff` / `comment_handoff`。也可以命令行直接验：
...
# ⇣ h_20260429_XXXXXXXX from user@backend → /path/to/test-frontend/.claude/handoff-inbox/h_...
```
...
打开前端 Claude Code（在 test-frontend 目录里），输入 `/pickup`。Claude 会：
...
```bash
ls /path/to/test-frontend/.claude/handoff-inbox/<id>/
...
仓库里的 `.cc-handoff.toml` 与 `.claude/handoff-inbox/` 自己看着删。
...
| `<repo>/.claude/handoff-inbox/<id>/` | 物化的 handoff（package.json、summary.md、prompt.md、full.diff、api-delta.md、comments.md、attachments/） |
| `<repo>/.claude/handoff-inbox/.draft-summary.md` | `/handoff` 时 Claude 写的草稿（每次 submit 都读，不自动清） |
