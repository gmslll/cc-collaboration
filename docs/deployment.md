...
实际就是 `claude mcp add --transport stdio cc-handoff -- /usr/local/bin/cc-handoff-mcp`，幂等（已存在时先 remove 再 add）。

### 2.4 安装 slash commands

把 `/handoff` 与 `/pickup` 拷到目标仓库：
...
> 后端仓库实际只用 `/handoff`，前端仓库实际只用 `/pickup`，但两份都拷无妨 — Claude 不会去调没意义的那条。
...
```bash
# 改 plist 里的 WorkingDirectory 指向接收侧仓库根
WORKDIR=/path/to/test-frontend

sed "s|/Users/CHANGE_ME/Workspace/your-frontend-repo|$WORKDIR|" \
  /path/to/cc-collaboration/scripts/launchd/com.cc-handoff.watch.plist \
