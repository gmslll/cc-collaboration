...
chmod +x cc-relay install.sh uninstall.sh rotate-token.sh backup.sh

# install.sh is idempotent: creates user/dirs/unit/sample tokens only when
# missing; always copies the binary and enables the service.
...
sudo install -m 0755 rotate-token.sh /usr/local/sbin/cc-handoff-rotate-token
...
echo "tokens  : /etc/cc-handoff/tokens.json   (edit then: systemctl restart cc-handoff-relay)"
echo "data    : /var/lib/cc-handoff/relay.db"
echo
echo "ops:"
echo "  sudo cc-handoff-rotate-token <identity>"
