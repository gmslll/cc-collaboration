...
void main() {
  ...
  test('chooseDistillStrategy: self only when idle AND user opted in', () {
    expect(
      chooseDistillStrategy(sessionIdle: true, userWantsSelf: true),
      DistillStrategy.selfDistill,
    );
    expect(
      chooseDistillStrategy(sessionIdle: false, userWantsSelf: true),
      DistillStrategy.offline,
    );
    expect(
      chooseDistillStrategy(sessionIdle: true, userWantsSelf: false),
      DistillStrategy.offline,
    );
  });

  test(
    'prompts name both draft files + marker (self) and ask stdout-only (offline)',
    () {
      final self = selfDistillPrompt(
        personaPath: '/d/persona.md',
        seedPath: '/d/seed.md',
      );
  ...
  test(
    'runOfflineDistill writes persona.md + seed.md from headless stdout',
    () async {
      final dir = draft();
  ...
  test(
    'runOfflineDistill returns false when the persona one-shot yields nothing',
    () async {
      final dir = draft();
      addTearDown(() => Directory(dir).deleteSync(recursive: true));
      final ok = await runOfflineDistill(
  ...
  test(
    'distillCapsule self path: idle+opt-in delivers to the live session',
    () async {
      final dir = draft();
      addTearDown(() => Directory(dir).deleteSync(recursive: true));
      var delivered = false;
  ...
  test(
    'distillCapsule self timeout → falls back to headless offline',
    () async {
      final dir = draft();
      addTearDown(() => Directory(dir).deleteSync(recursive: true));
      final out = await distillCapsule(
