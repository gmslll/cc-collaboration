...
void main() {
  ...
  group('inputDirty tracking', () {
    test('agent sessions use a wide quiet fallback; shell stays quick', () {
      final claude = TerminalSession('/repo', 'claude', agent: 'claude');
      final codex = TerminalSession('/repo', 'codex', agent: 'codex');
      final shell = TerminalSession('/repo', '', agent: '');
      addTearDown(claude.dispose);
      addTearDown(codex.dispose);
      addTearDown(shell.dispose);

      expect(claude.debugBootSettle, codex.debugBootSettle);
      expect(claude.debugBootSettle, greaterThan(shell.debugBootSettle));
      expect(shell.debugBootSettle, const Duration(milliseconds: 1500));
    });

    test('boot hook readiness ignores stale SessionStart events', () {
      final s = TerminalSession('/repo', 'claude', agent: 'claude');
      addTearDown(s.dispose);
      final bootAt = DateTime.utc(2026, 7, 7, 12);
      s.debugSetBootStartedAt(bootAt);

      expect(
        s.debugHasFreshSessionStart([
          HookActivity(
            at: bootAt.subtract(const Duration(minutes: 1)),
            event: 'SessionStart',
          ),
        ]),
        isFalse,
        reason:
            'events from a previous PTY with the same tsN must not ready it',
      );
      expect(
        s.debugHasFreshSessionStart([
          HookActivity(
            at: bootAt.add(const Duration(milliseconds: 50)),
            event: 'SessionStart',
          ),
        ]),
        isTrue,
    ...
    test(
      'user keystrokes set it; a submit (CR) clears it; empty is a no-op',
      () {
        final s = TerminalSession('/repo', 'claude', agent: 'claude');
        addTearDown(s.dispose);
  ...
  test('deliverLocalMessage routes dirty→enqueue, clean-idle→paste', () async {
    ...
    final r1 = host.deliverLocalMessage(
      LocalMsg(from, target.id, 'hello', true),
    );
    expect(r1, isNull);
    expect(
      inboxHasJson(target),
    ...
    final r2 = host.deliverLocalMessage(
      LocalMsg(from, target.id, 'world', true),
    );
    expect(r2, isNull);
    expect(
      inboxHasJson(target),
  ...
  test(
    ...
    () async {
      ...
      expect(
        target.started,
        isFalse,
      ); // dormant: pane never mounted / hidden tab
      expect(target.ready, isFalse);

      final r = host.deliverLocalMessage(
        LocalMsg('ts-sender', target.id, 'task', true),
      );
      expect(r, isNull);
      expect(
        target.started,
      ...
      expect(
        ...
        0,
        reason:
            'on settle the message is handed to paste+submit — no manual Enter',
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(
        target.pendingWakeCount,
        0,
        reason: 'auto-runs on settle, no manual Enter',
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      final r2 = host.deliverLocalMessage(
        LocalMsg('ts-b', target.id, 'second', true),
      );
      expect(r2, isNull);
