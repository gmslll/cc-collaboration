...
class RTCDataChannelWeb extends RTCDataChannel {
  RTCDataChannelWeb(this._jsDc) {
    // PATCH cc-handoff: RTCDataChannelInit.binaryType is not a WebRTC init
    // member, so setting it before createDataChannel does not affect the real
    // channel. Force ArrayBuffer on both created and remotely-created channels;
    // otherwise each Blob is converted asynchronously and ordered packet
    // callbacks can be delivered to the PTY reassembler out of order.
    _jsDc.binaryType = 'arraybuffer';
