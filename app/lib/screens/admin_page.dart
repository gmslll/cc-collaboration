...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _create() async {
    ...
    try {
      final pw = await widget.client.createUser(id,
          password: _password.text.trim(), isAdmin: _newAdmin);
      _identity.clear();
      _password.clear();
      ...
      Card(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('创建账号', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            TextField(
                controller: _identity,
                decoration: const InputDecoration(
                    labelText: 'identity(如 alex@frontend)', isDense: true)),
            const SizedBox(height: 8),
            TextField(
                controller: _password,
                decoration: const InputDecoration(
                    labelText: '初始密码(留空自动生成)', isDense: true)),
            Row(children: [
              Checkbox(
                  value: _newAdmin,
              ...
              child: ListTile(
                ...
                trailing: PopupMenuButton<String>(
                  onSelected: (v) {
                    switch (v) {
                      case 'admin':
                        _act(() => widget.client.setUserAdmin(u.identity, !u.isAdmin));
                      case 'disable':
                        _act(() =>
                            widget.client.setUserDisabled(u.identity, !u.disabled));
                      case 'reset':
                        _act(() async {
                          final pw = await widget.client.resetPassword(u.identity);
