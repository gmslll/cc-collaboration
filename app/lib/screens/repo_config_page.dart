...
class _RepoConfigPageState extends State<RepoConfigPage> {
      ...
      body: c == null
          ...
          : DecoratedBox(
              ...
              child: ListView(
                ...
                children: [
                  Text(
                    RepoConfig.pathFor(widget.projectPath),
  ...
  Widget _triggersCard(RepoConfig c) => _card('触发(自动起 agent / 行为)', [
    _switch(
      'auto_launch(收到自动起 agent)',
      c.autoLaunch,
      (v) => setState(() => c.autoLaunch = v),
    ),
    _switch(
      'auto_launch_normal(普通优先级也起)',
      c.autoLaunchNormal,
      (v) => setState(() => c.autoLaunchNormal = v),
    ),
    _switch(
      'auto_launch_on_alert(日志告警也起)',
      c.autoLaunchOnAlert,
      (v) => setState(() => c.autoLaunchOnAlert = v),
    ),
    _switch(
      'wake_on_comment(对方回复唤回会话)',
      c.wakeOnComment,
      (v) => setState(() => c.wakeOnComment = v),
    ),
    _switch(
      'launch_interactive(交互 REPL 而非一次性)',
      c.launchInteractive,
      (v) => setState(() => c.launchInteractive = v),
    ),
    _switch(
      'mute_user_presence(静音上下线通知)',
      c.muteUserPresence,
      (v) => setState(() => c.muteUserPresence = v),
    ),
    const SizedBox(height: 6),
    _dropdown('terminal_app', c.terminalApp, const [
      'terminal',
      'iterm2',
      'ghostty',
      'windows-terminal',
    ...
    _dropdown('launch_mode', c.launchMode, const [
      'window',
      'split',
    ], (v) => setState(() => c.launchMode = v)),
    _dropdown('ack_on_launch', c.ackOnLaunch, const [
      'never',
      'after_exit',
      'on_launch',
      'slash_pickup',
  ...
  Widget _linearCard(RepoConfig c) => _card('Linear 集成', [
    ...
    _text(
      'project_id(Linear 项目 UUID)',
      c.linearProjectId,
      (v) => c.linearProjectId = v,
    ),
    _text('default_labels(逗号分隔)', c.defaultLabels, (v) => c.defaultLabels = v),
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                autocorrect: false,
                style: const TextStyle(fontFamily: CcType.mono, fontSize: 12),
