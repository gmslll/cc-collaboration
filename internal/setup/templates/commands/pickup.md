...
3. **先确认团队 / 成员定向边界**：
   - 读 `package.json` 里的 `targeting` / `targeting_hints`（字段名以实际包为准）。如果包是发给 `project` / `org` 的团队级共享，按该团队/项目的上下文集成；不要把它当成只发给默认 partner 的个人私信。
   - 如果包同时指定了 `member`，先确认自己就是该成员，或确认当前任务确实是在代表该成员处理；否则停止并让用户选择正确接收人。
   - 不要因为本地配置里还有 `identity.partner(s)` 就扩大收件或影响范围。团队包里的 project/org/member 是更强约束。
