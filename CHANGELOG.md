...
- **会话胶囊(Session Capsule)—— 把一个会话冻结成可共享的专职会话**。在「会话总览」点某个 agent 会话卡片的 🧪「打成胶囊」:捕获逐字 transcript + 混合蒸馏出「专职角色(persona.md)」和「上下文摘要(seed.md)」→ 复查/编辑后按 **个人 / 公开** 发到新的「广场」页。
  - **广场**:按可见性浏览(公开=整个团队都看得到、个人=只有自己);拥有者可**编辑(可见性 / 说明)和删除**自己的胶囊。
  - **载入**(桌面):从广场把一个胶囊起成专职新会话,启动时自选 **①完整快照 / ②蒸馏角色** + 目标工具(Claude / Codex)+ 目标项目。同工具跨机走原样 `--resume`(拉逐字日志导入本机会话库);跨工具 / 跨机走中性上下文 seed。
  - **自带技能**:蒸馏会(两步:先保存对话、再问源会话)列出这活依赖的本地技能 / 脚本目录,复查时勾选后**随胶囊打包带走**,载入时自动解压到接收方 `~/.claude/skills/`,解决「队友机器上没有这个技能」;载入开场也会提醒上下文可能来自另一台机器、按名字找技能。
  - 需 relay 更新到本版(新增 `/v1/capsules` 广场列表 + 按团队可见性授权 + 胶囊提交只依赖 relay 连接、不要求仓库 partner)。

## [0.9.15] - 2026-07-06

### Fixed

- **Linear 导入现在能选「导入到 relay 团队项目」** — 以前 App 的 Linear 导入只创建**个人待办**(不传 relay 项目),而团队/Relay/「全部项目」视图只显示团队待办、排除个人,所以导入的 Linear 待办在团队看板里**根本不出现**。现在 Linear 配置弹窗多了「导入到 relay 项目」下拉(不选=个人待办);选了项目后导入的待办变成**团队待办**,在 团队/Relay/该项目下可见、也能指派给成员,导入完自动跳到该项目视图。注:已导入的旧个人副本不会自动迁移,重新带项目导入会新建团队副本。
- **新建的 relay 项目立刻出现在待办页** — `/v1/me` 只在登录时拉一次,导致在「项目」页新建的项目要**重启 App** 才出现在待办页的项目筛选下拉、Linear 导入目标、以及新建待办的项目选择里。现在进入待办页 / 下拉刷新 / 点刷新都会重新拉一次,新项目免重启即可见可选。
...
- **手机端待办「一键指派」补齐三模式** — 移动端不再只有「指派成员」,现在和桌面端一样支持「已有会话 / 新建会话 / 指派成员」。手机没有本地会话,所以「已有会话 / 新建会话」是**远程驱动已配对在线的桌面**:手机列出桌面正在开的会话把待办投递进去,或让桌面新建一个会话——待办的物化、投递、会话绑定全在桌面本地完成(复用桌面现成的指派逻辑),手机只当遥控器。桌面离线/未配对时自动降级为只显示「指派成员」并提示。纯 app 改动,无需 relay 部署;需手机与桌面都升级到本版。
- **会话胶囊** — 把一个会话打成「胶囊」,发到广场,或从广场载入。

### Changed

- **待办「已有会话」指派改成 workspace→project→会话 三级级联** 选择,和「新建会话」一致的体验。

### Fixed

- **恢复会话时补投任务说明**,避免恢复出的终端空白。
- **Linear「In Review」正确同步成「进行中」** — started 分支按 state.name 消歧。
